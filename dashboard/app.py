...
# ── Paths ─────────────────────────────────────────────────────────────────────
KNOWN_FACES_DIR = Path(__file__).parent.parent / "services" / "vision" / "known_faces"
...
with tab_feed:
    if st.session_state.mongo_error:
        st.error("⚠️ MongoDB connection failed — check your MONGODB_URI in .env")
    else:
        st.caption(f"✅ MongoDB connected · {len(st.session_state.events)} events loaded")
...
# Use meta-refresh via st.html for reliable auto-reload in all Streamlit versions
