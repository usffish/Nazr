"""
services/brain/routes/event.py — POST /event route.
...
from services.brain.models import Event, EventRecord, EventResponse
from services.brain.services.gemini import verify_health_event, generate_voice_script
from services.brain.services.elevenlabs import synthesize_audio
from services.brain.services.audio import play_audio
from services.brain.services.mongodb import write_event_record
...
async def handle_event(event: Event, request: Request) -> EventResponse:
    ...
    Runs the full pipeline synchronously before returning HTTP 200.
    All downstream failures are caught and absorbed — never propagates 5xx.

    Requirements: 1.1, 1.5, 1.6, 8.2, 8.3, 8.4
    """
    # Store event_id on request.state for the global exception handler
    ...
    verified = True  # default for identity events
    ...
    if any_failure:
        message = "Event processed with partial failures."
    else:
